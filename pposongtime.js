    ...
    for (const time of times) {
        ...
        for (const section of route.sections) {
            ...
            if (section.mode === 'WALK') {
                ...
                // db에서 RN1받아옴
                try {
                    const results = await dbQueryAsync('SELECT RN1 FROM FORECAST WHERE TIME = ? AND X = ? AND Y = ?', [base_time_str, X, Y]);
                    if (results.length > 1) {
        ...
        pposong_results2.push({ START_TIME: time.Time, END_TIME: end_time, RN1_SUM: RN1_sum, WTIME: WTIME });
    };
